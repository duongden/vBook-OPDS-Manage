...
const app = { request: (input: any, init?: any, env?: any, ctx?: any) => gateway.request(input, init, { ENABLE_LEGACY_ROUTES: 'true', ...env }, ctx) };
...
assert(
  osRes.headers.get('Cache-Control')?.includes('no-store') === true,
  'Legacy descriptions do not leak through shared caches'
);
...
assert(emptySearchWithAuthRes.status === 410, 'Retire caller-selected query authentication');
const authFailRes = await app.request('/feed/root_folder', {
  headers: { Authorization: 'Basic ' + btoa('user:wrongpassword') },
}, { AUTH_TOKEN: 'correctToken' });
assert(authFailRes.status === 401, 'Server credentials cannot be overridden');
