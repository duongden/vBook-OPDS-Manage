...
export function extractFolderId(input: string): string | null {
  ...
  const folderMatch = url.pathname.match(/\/folders\/([a-zA-Z0-9_-]{10,60})(?:\/|$)/);
...
async function executeDriveQuery(options: {
  ...
  const fields = 'nextPageToken, incompleteSearch, files(id, name, mimeType, size, modifiedTime, thumbnailLink, hasThumbnail)';
  ...
  if (data.incompleteSearch) throw new Error('DriveIncompleteSearch');
  ...
  while (currentLevelFolderIds.length > 0 && depth < maxDepth && collectedFolderIds.length < maxFolders) {
    ...
    for (let i = 0; i < currentLevelFolderIds.length; i += batchSize) {
      ...
      try {
        ...
        console.warn('Lỗi khi quét thư mục con:');
        break;
      }
    }
...
export async function getCachedDescendantFolderIds(options: {
  ...
  const digest = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(JSON.stringify([rootFolderId, apiKey, maxDepth, maxFolders])));
  const identity = Array.from(new Uint8Array(digest), b => b.toString(16).padStart(2, '0')).join('');
  for (const [key, entry] of memoryTreeCache) if (entry.expiresAt <= now) memoryTreeCache.delete(key);
  if (memoryTreeCache.size >= 256) memoryTreeCache.delete(memoryTreeCache.keys().next().value!);
  const memCached = memoryTreeCache.get(identity);
  if (memCached && memCached.expiresAt > now) {
    return memCached.folderIds;
  }

  // 2. Kiểm tra Cloudflare Edge Cache API (nếu có)
  const cacheKey = `https://internal.cache/tree/${identity}`;
  if (typeof caches !== 'undefined' && caches?.default) {
    try {
      const match = await caches.default.match(new Request(cacheKey));
      if (match) {
        const ids = (await match.json()) as string[];
        memoryTreeCache.set(identity, { folderIds: ids, expiresAt: now + ttlSeconds * 1000 });
        return ids;
      }
    } catch {
      // Bỏ qua lỗi cache
  ...
  // 4. Lưu Memory Cache
  memoryTreeCache.set(identity, { folderIds, expiresAt: now + ttlSeconds * 1000 });
...
export async function searchDriveFolder(options: {
  ...
  if (deepSearch) {
    try {
      ...
      console.warn('Lỗi quét deep search subfolders, fallback về search cấp 1:');
    }
  }
