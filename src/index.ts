...
import { libraryHtml, libraryCss } from './library-ui';
...
export interface Env {
  ...
  AUTH_TOKEN?: string;
  MASK_SECRET?: string;
  ENABLE_LEGACY_ROUTES?: string;
}

const app = new Hono<{ Bindings: Env }>();

// Managed library routes own their authentication and do not inherit legacy CORS/auth.
app.route('/', libraryApp);
app.get('/assets/library.css', c => c.body(libraryCss, 200, { 'Content-Type': 'text/css;charset=utf-8', 'X-Content-Type-Options': 'nosniff' }));
app.get('/assets/library.js', c => c.body(libraryClient, 200, { 'Content-Type': 'text/javascript;charset=utf-8', 'X-Content-Type-Options': 'nosniff' }));

// Kích hoạt CORS cho mọi nguồn
// Legacy URLs cannot provide per-library authorization. Keep them explicitly opt-in.
app.use('*', async (c, next) => {
  if (/^\/(?:legacy|api\/mask|feed|download)(?:\/|$)/.test(c.req.path)) {
    c.header('Cache-Control', 'private, no-store');
    c.header('Referrer-Policy', 'no-referrer');
    c.header('X-Content-Type-Options', 'nosniff');
    if (c.env?.ENABLE_LEGACY_ROUTES !== 'true') return c.text('Đường dẫn legacy đã tắt. Hãy tạo thư viện có tài khoản tại trang chủ.', 404);
    if (c.req.query('auth') !== undefined) return c.text('Link dùng ?auth= đã ngừng hỗ trợ vì không bảo vệ được thư viện. Hãy tạo link OPDS mới tại trang chủ.', 410);
    if (!isAuthorized(c)) return c.text('Yêu cầu xác thực HTTP Basic Auth của máy chủ.', 401, { 'WWW-Authenticate': 'Basic realm="vBook Legacy"' });
  }
  await next();
});
app.use('*', cors());
...
async function getFromEdgeCache(c: any, isOpds2: boolean): Promise<Response | null> {
  try {
    ...
    console.warn('Lỗi đọc Edge Cache');
  }
  return null;
}
...
async function putToEdgeCache(c: any, isOpds2: boolean, res: Response): Promise<void> {
  try {
    ...
    console.warn('Lỗi ghi Edge Cache');
  }
}

/**
 * Kiểm tra xác thực Basic Auth:
 * Chỉ dùng credential do máy chủ cấu hình. Tham số ?auth= bị từ chối trước route.
...
function isAuthorized(c: any): boolean {
  ...
  if (!envToken && (c.env?.AUTH_USER || c.env?.AUTH_PASS)) return false;
  const expectedToken = envToken;
  if (!expectedToken) {
...
app.get('/', (c) => {
  ...
  c.header('Content-Security-Policy', "default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self' https:; connect-src 'self'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'self'");
  return c.html(c.env?.ENABLE_LEGACY_ROUTES === 'true' ? libraryHtml : libraryHtml.replace('<a href="/legacy">Tạo link nhanh</a>', ''));
});
...
app.post('/api/mask', async (c) => {
  try {
    ...
    const rawInput = typeof body?.folderId === 'string' ? body.folderId.trim() : '';
...
app.get('/feed/:folderId', async (c) => {
  ...
  try {
    ...
    console.error('Lỗi lấy dữ liệu Drive');
...
app.get('/feed/:folderId/opensearch.xml', async (c) => {
  ...
  return c.text(xml, 200, {
    ...
    'Cache-Control': c.env?.AUTH_TOKEN || c.env?.AUTH_USER ? 'private, no-store' : 'public, max-age=86400',
  });
});
...
app.get('/feed/:folderId/search', async (c) => {
  ...
  try {
    ...
    console.error('Lỗi tìm kiếm Drive');
    const msg = error.message || '';
    if (
      msg.includes('429') ||
      msg.includes('rateLimitExceeded') ||
