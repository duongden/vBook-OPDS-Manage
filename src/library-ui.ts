...
<dialog id="sources"><div class="dialog-head"><div><p class="eyebrow">CATALOG TỔNG HỢP</p><h2>Nguồn sách</h2></div><button class="icon-button" data-close="sources" aria-label="Đóng" title="Đóng"><svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M6 6l12 12M18 6L6 18"/></svg></button></div><section class="drive-source"><h3>Google Drive</h3><p id="drive-status" class="muted">Chưa kết nối thư mục Drive.</p><form id="drive-form"><label>Link thư mục Google Drive hoặc Folder ID <small>Thư mục cần chia sẻ “Bất kỳ ai có đường liên kết” với quyền xem.</small><input name="drive" required maxlength="2048" autocomplete="off" placeholder="https://drive.google.com/drive/folders/..."></label><div class="drive-actions"><button class="primary" id="drive-save">Thêm thư mục Drive</button><button class="danger" id="drive-remove" type="button" hidden>Gỡ nguồn Drive</button></div></form></section><hr><h3>Nguồn OPDS</h3><p class="muted">Mỗi URL nằm trên một dòng. Tài khoản bên dưới được dùng chung cho các URL trong lần thêm này và được mã hóa trước khi lưu.</p><form id="source-form"><label>Danh sách URL OPDS<textarea name="urls" rows="4" maxlength="12000" required placeholder="https://catalog.example/opds&#10;https://books.example/feed.xml"></textarea></label><p class="source-auth-help muted">Để trống cả hai ô nếu nguồn công khai.</p><div class="form-grid source-credentials"><label>Username nguồn<input name="username" maxlength="256" autocomplete="off"></label><label>Mật khẩu nguồn<input name="password" type="password" maxlength="512" autocomplete="new-password"></label></div><button class="primary">Thêm và kiểm tra nguồn</button></form><hr><div id="source-list" class="source-list" aria-live="polite"><p class="muted">Chưa có nguồn OPDS.</p></div></dialog>
...
.drive-source h3 { margin-bottom: var(--space-1); }
.drive-source form { margin-top: var(--space-3); }
