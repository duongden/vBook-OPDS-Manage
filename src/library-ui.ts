...
<body><header><div class="wrap header"><a href="/" class="brand"><span class="mark">V</span><span>VBook Library<small>GOOGLE DRIVE TO OPDS</small></span></a><nav aria-label="Điều hướng"><a href="/legacy">Tạo link nhanh</a><button id="theme-toggle" type="button" aria-label="Chuyển sang giao diện tối" aria-pressed="false">☾</button><button id="account-button" hidden>Tài khoản</button><button id="logout" hidden>Đăng xuất</button></nav></div></header>
...
.view-toggle button { height: var(--control-height); min-height: var(--control-height); padding-block: 0; }
