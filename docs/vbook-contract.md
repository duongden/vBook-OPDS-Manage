...
- IDs sách dùng HMAC ổn định theo thư viện, không chứa File ID thô. Link ảnh ngoài không đoán MIME khi chỉ biết URL.
- Header mới: `private, no-store`, `Vary: Accept, Authorization`. Cache phía vBook có thể cần làm mới.
- Search OPDS vẫn tìm tên file Drive trong phạm vi legacy (3 cấp/35 thư mục con), chưa tìm metadata sửa. Quét toàn kho web là luồng khác.

Các đặc tả `/feed/:folderId`, `?auth=` phía dưới là contract **legacy**, giữ riêng để tương thích link cũ. Test giả lập không thay kiểm chứng vBook thật.
